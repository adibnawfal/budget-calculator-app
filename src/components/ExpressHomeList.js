...
export default function ExpressHomeList({
  navigation,
  express,
  item,
  index,
  ...
  const leftActions = (dragX, item, index) => {
    ...
    return (
      <TouchableOpacity
        onPress={() => {
          navigation.navigate("EditExpressHome", { express, item, index });
  ...
  return (
    ...
    >
      <View style={styles.listWrap}>
        <View style={{ flex: 1 }}>
          <Text style={styles.listTxt}>{item.itemName}</Text>
          <Text style={[styles.listTxt, { color: colors.darkGrey }]}>
            Quantity {item.qty}
          </Text>
        </View>
